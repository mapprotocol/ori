...
use self::helper::{DummyScoring, NonceReady,TransactionBuilder};
use std::sync::Arc;

use super::*;
use map_core::types::{Hash as H256,Address};
...
fn should_construct_pending() {
	...
	assert_eq!(txq.status(NonceReady::new(1 as u64)), Status { stalled: 3, pending: 6, future: 2 });

	// when
	let mut current_gas = 0 as u64;
	let limit = (21_000 * 8) as u64;
...
fn should_skip_staled_pending_transactions() {
	...
	let mut pending = txq.pending(NonceReady::new(2 as u64));
...
fn should_cull_stalled_transactions() {
	...
	assert_eq!(txq.status(NonceReady::new(1 as u64)), Status { stalled: 2, pending: 2, future: 2 });

	// when
	assert_eq!(txq.cull(None, NonceReady::new(1 as u64)), 2);

	// then
	assert_eq!(txq.status(NonceReady::new(1 as u64)), Status { stalled: 0, pending: 2, future: 2 });
...
fn should_cull_stalled_transactions_from_a_sender() {
	...
	assert_eq!(txq.status(NonceReady::new(2 as u64)), Status { stalled: 4, pending: 1, future: 0 });

	// when
	let sender = Address::zero();
	assert_eq!(txq.cull(Some(&[sender]), NonceReady::new(2 as u64)), 2);

	// then
	assert_eq!(txq.status(NonceReady::new(2 as u64)), Status { stalled: 2, pending: 1, future: 0 });
...
fn should_re_insert_after_cull() {
	...
	import(&mut txq, b.tx().sender(1).nonce(0).new()).unwrap();
	import(&mut txq, b.tx().sender(1).nonce(1).new()).unwrap();
	assert_eq!(txq.status(NonceReady::new(1 as u64)), Status { stalled: 2, pending: 2, future: 0 });

	// when
	assert_eq!(txq.cull(None, NonceReady::new(1 as u64)), 2);
	assert_eq!(txq.status(NonceReady::new(1 as u64)), Status { stalled: 0, pending: 2, future: 0 });
	import(&mut txq, b.tx().nonce(0).gas_price(5).new()).unwrap();
	import(&mut txq, b.tx().sender(1).nonce(0).new()).unwrap();
...
fn should_return_worst_transaction() {
	...
	// then
	assert_eq!(txq.worst_transaction().unwrap().gas_price, 4 as u64);
}
...
fn should_not_import_even_if_limit_is_reached_and_should_replace_returns_false() {
	...
	assert_eq!(
		...
		error::Error::TooCheapToEnter(
			H256::from_hex("00000000000000000000000000000000000000000000000000000000000001f5").unwrap(),
			"0x5".into()
		)
	);
	assert_eq!(txq.light_status(), LightStatus { transaction_count: 1, senders: 1, mem_usage: 0 });
...
mod listener {
	...
	fn cull_stalled() {
		...
		// when
		txq.cull(None, NonceReady::new(3 as u64));
